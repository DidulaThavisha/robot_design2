...
def time_separation(df_dir):
    df = pd.read_csv(df_dir)
    df_first_visit = pd.DataFrame(columns=df.columns)
    df_last_visit = pd.DataFrame(columns=df.columns)

    for i in range(0,len(df)):
        path = df.iloc[i,0].split('/')
        if(path[1] == 'Prime_FULL'):
            visit_string = path[3]
            if (int(visit_string[1]) == 0 and len(visit_string) == 2):
                df_first_visit.loc[len(df_first_visit)] = df.iloc[i]
            else:
                df_last_visit.loc[len(df_last_visit)] = df.iloc[i]
        else:
            visit_string = path[4]
            if(int(visit_string[1]) ==1 and len(visit_string) == 2):
                df_first_visit.loc[len(df_first_visit)] = df.iloc[i]
            else:
                df_last_visit.loc[len(df_last_visit)] = df.iloc[i]

    df_first_visit.to_csv('/home/kiran/Desktop/Dev/SupCon_OCT_Clinical/final_csv_time_ood_analysis/time_analysis/testing_first_visit.csv',index=False)
    df_last_visit.to_csv('/home/kiran/Desktop/Dev/SupCon_OCT_Clinical/final_csv_time_ood_analysis/time_analysis/testing_last_visit.csv',index=False)

def counts(df_dir):
    df = pd.read_csv(df_dir)
    print(df['Fluid (IRF)'].value_counts())
    print(df['DRT/ME'].value_counts())
    print(df['IR HRF'].value_counts())
    print(df['Partially attached vitreous face'].value_counts())
    print(df['Fully attached vitreous face'].value_counts())

def test_time_split(df_dir,att,q):
    df = pd.read_csv(df_dir)
    df_0 = df[df[att] == 0]
    df_1 = df[df[att] == 1]

    test_0 = df_0.sample(n=q, random_state=32)
    test_1 = df_1.sample(n=q, random_state=32)
    test = pd.concat([test_0, test_1])
    test.to_csv('/home/kiran/Desktop/Dev/SupCon_OCT_Clinical/final_csv_time_ood_analysis/time_analysis/testing_time_first_biomarkers/' + att[0:3] + '.csv',index=False)
def clin_trial_separation(df_dir):
    df = pd.read_csv(df_dir)
    df_prime = pd.DataFrame(columns=df.columns)
    df_trex = pd.DataFrame(columns=df.columns)

    for i in range(0, len(df)):
        path = df.iloc[i, 0].split('/')
        if (path[1] == 'Prime_FULL'):
            df_prime.loc[len(df_prime)] = df.iloc[i]
        else:
            df_trex.loc[len(df_trex)] = df.iloc[i]

    df_prime.to_csv(
        '/home/kiran/Desktop/Dev/SupCon_OCT_Clinical/final_csv_time_ood_analysis/ood_analysis/training_prime.csv',
        index=False)
    df_trex.to_csv(
        '/home/kiran/Desktop/Dev/SupCon_OCT_Clinical/final_csv_time_ood_analysis/ood_analysis/training_trex.csv',
        index=False)
def reduce_trex(df_dir,q):
    df = pd.read_csv(df_dir)
